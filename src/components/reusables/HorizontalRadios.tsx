...
const HorizontalRadios: React.FC<{
  radioValues: RadioProps[];
  onChange: any;
  ...
  return (
    <div className="flex items-center relative w-full justify-between ">
      ...
      {radioValues?.map((values: RadioProps) => (
        <Radio {...values} onChange={onChange} />
      ))}
      {/* </div> */}
    </div>
  );
...
const Radio: React.FC<RadioProps & { onChange: any }> = ({
  label,
  value,
  checked,
  onChange,
  ...
  return (
    <div className="flex flex-col gap-2 justify-center items-center">
      <div className="bg-black dark:bg-black rounded-full w-6 h-6 flex 
