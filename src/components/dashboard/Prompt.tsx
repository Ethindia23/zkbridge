...
export function Prompt({ note, onSubmit, onCancel }: PromptValue) {
  return (
    <div className=" fixed top-0 left-0 right-0 bottom-0 w-[100vw] h-[100vh] backdrop-blur-sm bg-black/20">
      <div
        className="flex flex-col gap-4 absolute z-50  top-[30%] left-[28%]  w-[40rem] p-6 py-6
    
    shadow-3xl bg-slate-900
              text-zinc-200 "
      >
        <span className="text-lg text-zinc-200 font-semibold underline underline-offset-4">
          Your private note
        </span>
        <span
          onClick={() => onCancel()}
        ...
        <span className="text-base font-[500]  text-teal-500 flex gap-2 items-center cursor-pointer">
          {note.substring(0,55)}...
          <span
            onClick={() => {
          ...
          >
            <BiCopyAlt color="rgb(20 184 166)" size={20} />
          </span>
        </span>
        <button
          onClick={() => onSubmit()}
